//! Test script for the Marketstack provider with live API.
//!
//! Run with: cargo run --example marketstack_test
//!
//! Requires the API key to be stored in pass at `keepbook/marketstack-api-key`.
...
fn get_api_key_from_pass() -> anyhow::Result<String> {
    let output = Command::new("pass")
        .arg("show")
        .arg("keepbook/marketstack-api-key")
        .output()?;
...
async fn main() -> anyhow::Result<()> {
    ...
    match provider.fetch_close(&asset, &asset_id, test_date).await {
        ...
        Ok(None) => {
            ...
            println!("This might happen if the market was closed or data is not available.");
        }
        Err(e) => {
            println!("ERROR: {e}");
            return Err(e);
