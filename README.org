...
trait Storage {
    ...
    fn get_connection(&self, id: &Id) -> Result<Connection>;

    // Accounts
    fn list_accounts(&self) -> Result<Vec<Account>>;
    fn get_account(&self, id: &Id) -> Result<Account>;

    // Balances
    fn get_balances(&self, account_id: &Id, range: TimeRange) -> Result<Vec<Balance>>;

    // Transactions
    fn get_transactions(&self, account_id: &Id, range: TimeRange) -> Result<Vec<Transaction>>;

    // Writes (batch-only)
    fn write_batch(&self, metadata: &BatchMetadata, updates: &[AccountUpdate]) -> Result<()>;
}
#+END_SRC
...
| Field | Description |
|-------+-------------|
| id | Opaque string id (often UUID), immutable |
