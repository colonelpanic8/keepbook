...
- =auth=
  ...
  - =chase login=
- =market-data fetch=
- =portfolio snapshot|history|change-points|graph=
- =spending=
...
# Build an ad-hoc static net-worth graph
keepbook portfolio graph \
  --start 2026-03-01 \
  --end 2026-04-01 \
  --granularity daily \
  --output artifacts/net-worth-march.html
#+END_SRC

The graph command writes an SVG and a small HTML wrapper, then prints JSON with
the generated paths. For repeatable ad-hoc runs, pass =--graph-config <path>=
with TOML fields matching the flags, for example:
...
history_points = 8
# Spending lookback windows (in days) shown in the Recent Spending tray rows.
