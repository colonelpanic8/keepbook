...
///
/// The parent and its worker processes coordinate through files, and on a
/// loaded machine there is no upper bound on how long that takes. A fixed
/// number of short sleeps turns ordinary slowness into a failure; a deadline
/// only fires when the signal never arrives at all.
async fn wait_until(
    budget: Duration,
    what: &str,
...
async fn persistence_worker() -> Result<()> {
    ...
    wait_until(Duration::from_secs(120), "the start signal", || {
...
async fn processes_append_safely_while_other_stores_read_and_compact() -> Result<()> {
    ...
    wait_until(
        ...
        "every worker to report ready",
        || (0..3).all(|worker| dir.path().join(format!("ready-{worker}")).exists()),
    )
    .await?;
    ...
    for child in children {
        let output = tokio::time::timeout(Duration::from_secs(120), child.wait_with_output())
            .await
            .context("timed out waiting for a persistence worker to exit")??;
        anyhow::ensure!(
