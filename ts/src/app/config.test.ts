...
import { DEFAULT_GIT_CONFIG, DEFAULT_REFRESH_CONFIG } from "../config.js";
...
describe("loadConfig", () => {
  ...
  it("returns defaults when config file does not exist", async () => {
    ...
    expect(config.refresh.balance_staleness).toBe(
      DEFAULT_REFRESH_CONFIG.balance_staleness,
    );
    expect(config.refresh.price_staleness).toBe(
...
describe("configOutput", () => {
  ...
  it("does not include extra fields like reporting_currency or refresh", () => {
    ...
    expect(Object.keys(result).sort()).toEqual(
      ["config_file", "data_directory", "git"].sort(),
    );
  });
});
